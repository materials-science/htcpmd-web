<template>
	<d2-container :class="{ 'page-compact': crud.pageOptions.compact }">
		<template slot="header"
			>Structure List
			<!-- <example-helper title="Help Document">
				<div>
					Structure List
					<link-button
						href="http://greper.gitee.io/d2-crud-plus/guide/permission.html"
						>Help Document</link-button
					>
				</div>
			</example-helper> -->
		</template>
		...
		<el-dialog title="授予角色" :visible.sync="dialogPermissionVisible">
			...
			<div slot="footer" class="dialog-footer">
				<el-button
					type="primary"
					@click="updatePermession(currentUserId)"
					>确定
				</el-button>
			</div>
		</el-dialog>
...
<script>
	...
	import document, { d2CrudPlus } from "d2-crud-plus";
		...
		methods: {
			...
			authzHandle(event) {
				api.GetRoleList().then((ret) => {
					...
					const checkedRoleIds = roleList
						.map((item) => item.id)
						.filter((item) => userRoles.indexOf(item) >= 0);
